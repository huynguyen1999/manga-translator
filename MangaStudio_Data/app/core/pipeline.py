...
class Pipeline:
    ...
    def run(self, job, output_path, config_dict, log_callback, is_verbose=False, output_format='png'):
        ...
        try:
            ...
            if job['settings'].get('processing_device') in ('NVIDIA GPU', 'Apple GPU (MPS)', 'Auto'):
    ...
    def run_single_image_test(self, test_image_path, output_path, config_dict, log_callback, is_verbose=False):
        ...
        try:
            ...
            if config_dict.get('processing_device') in ('NVIDIA GPU', 'Apple GPU (MPS)', 'Auto'):
                command.append("--use-gpu")
    ...
    def _execute_subprocess(self, log_callback, command):
        ...
        if sys.platform == 'darwin':
            # 0.0 disables the MPS hard limit; keep headroom by default.
            my_env.setdefault(
                "PYTORCH_MPS_HIGH_WATERMARK_RATIO",
                os.getenv("MANGA_MPS_HIGH_WATERMARK_RATIO", "0.95"),
            )
