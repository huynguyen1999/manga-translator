...
@pytest.mark.asyncio
async def test_merged_confidence_is_independent_of_other_regions():
    lines = [
        [[10, 10], [40, 10], [40, 30], [10, 30]],
        [[160, 70], [190, 70], [190, 90], [160, 90]],
    ]
    textlines = [
        Quadrilateral(np.array(lines[0]), '', 0.8),
        Quadrilateral(np.array(lines[1]), '', 0.6),
    ]

    regions = await dispatch_merge(textlines, 200, 100)
