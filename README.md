...
When the desktop GUI is unavailable, run the lightweight web studio:

```bash
./run-macos-web.sh
```

Open the address printed by the frontend (normally `http://localhost:6868`).
This uses the storage configured in `.env` (PostgreSQL by default in the sample
configuration). Set `PERSISTENCE_MODE=filesystem` explicitly if PostgreSQL is
not available.
...
# Colorize black and white pages only (without translation):
$ python -m manga_translator local -i <path> --colorize-only

# Translate AND colorize black and white pages:
$ python -m manga_translator local -i <path> --colorizer mc2

# Translate every manga archive matched by a wildcard. Each archive produces
# <name>-translated.cbz (or .zip) next to the input files.
$ python -m manga_translator local -i './manga/*.cbz'
# Or pass archives explicitly:
...
```bash
python server/main.py --use-gpu --workers 4
```

In-process workers share local models. Loading, inference, and unloading take turns on one model thread, including on Apple Silicon; CPU pipeline work and remote translation requests can overlap. Different model presets load their own models, and each active image still needs its own buffers. Lower `--workers` if those buffers consume too much memory. Separate worker processes are used only when explicitly selecting `--executor-mode subprocess`.

### Semantic Search Lab

Select manga, embed their summaries and original pages locally, and compare ranked
summary, image, and combined search results in the workspace's **Search Lab**.
See [Search Lab setup and evaluation](docs/search-lab.md) for the optional Qdrant
service, model downloads, resumable indexing, and score definitions.
...
-i, --input INPUT [INPUT ...] Image/folder, ZIP/CBZ, or wildcard path (required)
...
--workers WORKERS     Number of active image pipelines (default: 3)
--use-gpu             Turn on gpu (auto switch between mps and cuda, default: enabled)
--no-gpu              Disable gpu usage and run on cpu
--use-gpu-limited     Turn on gpu excluding offline translator
...
#### Colorizer Options
```
colorization_size The size of the image to use for colorization (default 576). Set to -1 to use the full image size
denoise_sigma     Used for colorizer and affects color intensity, ranging from 0 to 255 (default 25). -1 to disable
colorizer         The colorization model to use (e.g. mc2, none)
color_threshold   Threshold / tolerance for detecting if image is already colored (default 31.0). Skips colorization on color pages. -1 to disable
restore_size      Whether to restore the colorized image to original dimensions with sharp lineart (default True)
exclude_color_pages Pages to exclude from coloring (e.g. 1,2,5-8, or patterns like *cover*)
```
...
| `GEMINI_API_KEY`                       | Gemini API Key (supports multiple comma-separated keys for rotation)                                     | `''`                               | e.g. `'key1,key2,key3'`                                                                                   |
| `GEMINI_API_KEYS`                      | Gemini API Keys (alternative multi-key list for automatic rotation)                                      | `''`                               | e.g. `'key1, key2, key3'`                                                                                 |
...
| `OPENROUTER_API_KEY`                   | OpenRouter API Key                                                                        | `''`                               | Get key at https://openrouter.ai/settings/keys                                                             |
| `OPENROUTER_API_BASE`                  | OpenRouter API Base URL                                                                   | `https://openrouter.ai/api/v1`     | Defaults to official OpenRouter API endpoint                                                               |
| `OPENROUTER_MODEL`                     | OpenRouter Model Name                                                                     | `deepseek/deepseek-v4-flash-0731`  | Default model requested                                                                                    |
| `OPENROUTER_PROVIDER_SORT`             | OpenRouter Provider Routing Strategy                                                      | `price`                            | Routes requests to the cheapest provider                                                                   |
