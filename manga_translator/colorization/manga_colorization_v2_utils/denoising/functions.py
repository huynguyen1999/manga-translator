...
def concatenate_input_noise_map(input, noise_sigma):
    ...
    # Fill the downsampled image with zeros matching input device and dtype
    downsampledfeatures = torch.zeros((N, Cout, Hout, Wout), dtype=input.dtype, device=input.device)

    # Build the CxH/2xW/2 noise map on the same device
    noise_map = noise_sigma.to(input.device).view(N, 1, 1, 1).repeat(1, C, Hout, Wout)
...
class UpSampleFeaturesFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        # Build output
        grad_input = torch.zeros((N, Cg_in, Hg_in, Wg_in), dtype=grad_output.dtype, device=grad_output.device)
        # Populate output
        for idx in range(sca2):
