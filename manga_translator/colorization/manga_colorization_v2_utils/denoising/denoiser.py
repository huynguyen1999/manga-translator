...
class FFDNetDenoiser:
    ...
    def load_weights(self):
        ...
        if self.device == 'cuda':
            ...
            # MPS, XPU and CPU devices don't support DataParallel
            state_dict = torch.load(weights_path, map_location=self.device)
            # CPU/MPS mode: remove the DataParallel wrapper
    ...
    def get_denoised_image(self, imorig, sigma = None):
        ...
        imorig = torch.from_numpy(imorig.copy()).float().to(self.device)
        imnoisy = imorig
