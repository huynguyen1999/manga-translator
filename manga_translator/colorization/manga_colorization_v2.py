...
logger = get_logger('colorizer')


from .detector import is_image_colored, distance_from_grayscale


def transfer_color_to_original(original: Image.Image, colorized: Image.Image) -> Image.Image:
    """
    Transfers the color (chrominance) from the colorized image onto the original image's
    luminance channel in LAB color space. This retains 100% of the original image's resolution,
    crisp lineart, hatching, screentones, and text sharpness.
    """
    orig_rgb = np.array(original.convert('RGB'))
    if original.size == colorized.size:
        col_rgb = np.array(colorized.convert('RGB'))
    else:
        colorized_resized = colorized.resize(original.size, Image.Resampling.BICUBIC)
        col_rgb = np.array(colorized_resized.convert('RGB'))

    orig_lab = cv2.cvtColor(orig_rgb, cv2.COLOR_RGB2LAB)
    col_lab = cv2.cvtColor(col_rgb, cv2.COLOR_RGB2LAB)
    # Replace L (lightness) of colorized with original L
    col_lab[:, :, 0] = orig_lab[:, :, 0]
    restored = cv2.cvtColor(col_lab, cv2.COLOR_LAB2RGB)
...
class MangaColorizationV2(OfflineColorizer):
    ...
    async def _infer(self, image: Image.Image, colorization_size: int, denoise_sigma=25, **kwargs) -> Image.Image:
        # Check if the page is already colored (e.g. color cover or color spread)
        color_threshold = kwargs.get('color_threshold')
        if color_threshold is None:
            color_threshold = kwargs.get('color_tolerance', 31.0)
        if color_threshold is not None and color_threshold > 0:
            is_col, col_details = is_image_colored(image, color_threshold=color_threshold)
            if is_col:
        ...
        colored_image_arr = (result.numpy() * 255.0).clip(0, 255).astype(np.uint8)
        colored_image_pil = Image.fromarray(colored_image_arr)

        # Restore original image resolution while maintaining sharp lineart via LAB luminance replacement
        restore_size = kwargs.get('restore_size', True)
        if restore_size:
            colored_image_pil = transfer_color_to_original(image, colored_image_pil)

        return colored_image_pil
