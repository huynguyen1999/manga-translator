"""Small, file-backed diagnostics for the local Pipeline Lab UI."""
...
ACTIVE_RUNS: dict[str, PipelineLabRun] = {}
...
async def save_result_documents(folder: str, documents: dict[str, Any]) -> None:
    ...
    root = Path(SERVER_RESULT_ROOT).resolve()
...
class PipelineLabRun:
    def __init__(self, result_root: str | Path, folder: str, image, config):
        ...
        self.manual = bool(getattr(getattr(config, "pipeline_lab", None), "manual", False))
        ...
        plan = dict(getattr(getattr(config, "pipeline_lab", None), "stage_plan", {}) or {})
        active_plan = plan.copy()
        colorizer = getattr(getattr(config, "colorizer", None), "colorizer", None)
        if getattr(colorizer, "value", colorizer) == "none":
            active_plan["colorization"] = False
        if not getattr(getattr(config, "upscale", None), "upscale_ratio", None):
            active_plan["upscaling"] = False
        ...
        for stage_id, label in STAGES:
            ...
            elif not active_plan.get(stage_id, True):
                status, reason = "skipped", "Disabled in pipeline plan"
            else:
    ...
    @classmethod
    def from_folder(cls, result_root: str | Path, folder: str) -> PipelineLabRun | None:
        if folder in ACTIVE_RUNS:
            return ACTIVE_RUNS[folder]
        return None

    @classmethod
    def get_or_load(cls, result_root: str | Path, folder: str) -> PipelineLabRun | None:
        if folder in ACTIVE_RUNS:
            return ACTIVE_RUNS[folder]
    ...
    def from_documents(
        ...
        run.started = {}
        run.manual = bool(manifest.get("manual", False))
        run._continue = threading.Event()
    ...
    def _begin(self, stage_id: str):
        ...
        current_index = next(index for index, item in enumerate(self.manifest["stages"]) if item["id"] == stage_id)
        for previous in self.manifest["stages"][:current_index]:
            if previous["status"] == "pending":
                self._finish(previous["id"], "skipped", "No work was reported for this phase")
    ...
    async def wait_for_continue(
        self,
        stage_id: str,
        notify: Callable[[str, bool], Awaitable[None]],
    ):
        if not self.manual or self._stage(stage_id)["status"] == "skipped":
            return
        if self.active_stage:
            self._finish(self.active_stage)
        stage = self._stage(stage_id)
        stage["reason"] = "Waiting for Continue"
        self.manifest["status"] = "paused"
        self.manifest["waitingFor"] = stage_id
        self.refresh()
        await notify(f"manual_wait:{stage_id}", False)
        try:
            while not self._continue.is_set():
                if self.is_stop_requested():
                    self.cancel("Pipeline stopped by user")
    ...
    async def retry_stage(
        ...
        try:
            if stage_id == "colorization":
                ...
                ctx.text_mask = await translator._run_mask_refinement(config, ctx)
    ...
    async def retry_from_stage(
        ...
        result = None
        for current_stage in stage_ids[start:]:
            if current_stage == "input" or self._stage(current_stage).get("status") == "skipped":
                continue
