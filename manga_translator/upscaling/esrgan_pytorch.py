...
class ESRGANUpscalerPytorch(OfflineUpscaler):
    ...
    def _infer_tiled(self, image: Image.Image) -> Image.Image:
        """Run UltraSharp on bounded tiles so full manga pages do not exhaust memory."""
        source = np.array(image.convert('RGB'))[:, :, ::-1].copy()
        height, width = source.shape[:2]
        scale = getattr(self, 'scale', 4)
        result = np.empty((height * scale, width * scale, 3), dtype=np.float32)

        for top in range(0, height, self._TILE_SIZE):
            for left in range(0, width, self._TILE_SIZE):
                bottom = min(top + self._TILE_SIZE, height)
                right = min(left + self._TILE_SIZE, width)
                padded_top = max(0, top - self._TILE_PAD)
                padded_left = max(0, left - self._TILE_PAD)
                padded_bottom = min(height, bottom + self._TILE_PAD)
                padded_right = min(width, right + self._TILE_PAD)
                tile = torch.from_numpy(source[padded_top:padded_bottom, padded_left:padded_right]).float().div(255).permute(2, 0, 1).unsqueeze(0).to(self.device)
                with torch.no_grad():
                    output = self.model(tile)[0].clip(0, 1).cpu().numpy().transpose(1, 2, 0)
                ...
                result[top * scale:bottom * scale, left * scale:right * scale] = output[crop_top:crop_bottom, crop_left:crop_right]

        return Image.fromarray((result[:, :, ::-1] * 255).astype(np.uint8))
