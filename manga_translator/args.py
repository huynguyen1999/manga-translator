...
def expand_input_paths(inputs):
    paths = []
    for value in inputs:
        pattern = url_decode(os.path.expanduser(value))
        matches = glob.glob(pattern, recursive=True) if glob.has_magic(pattern) else [pattern]
        if not matches:
            raise FileNotFoundError(f'No file or directory matches: "{value}"')
        paths.extend(matches)

    unique_paths = list(dict.fromkeys(os.path.abspath(path) for path in paths))
    missing = [path for path in unique_paths if not os.path.exists(path)]
    if missing:
...
def general_parser(g_parser):
    ...
    g_parser.add_argument('--batch-size', default=20, type=int,
    ...
    g_parser.add_argument('--colorizer', default=None, choices=[c.value for c in Colorizer],
                        help='AI model to colorize black and white manga pages (e.g. mc2, none)')
    g_parser.add_argument('--colorization-size', default=None, type=int,
                        help='Base resolution for colorization (default 576)')
    g_parser.add_argument('--denoise-sigma', default=None, type=int,
                        help='Strength of denoising filter during colorization (0-255, default 25, -1 to disable)')
    g_parser.add_argument('--color-threshold', '--color-tolerance', dest='color_threshold', default=None, type=float,
                        help='Threshold / tolerance for skipping already colored pages (default 31.0, -1 to disable)')
    g_parser.add_argument('--restore-size', action=argparse.BooleanOptionalAction, default=None,
                        help='Restore colorized image to original dimensions with sharp lineart (default True)')
    g_parser.add_argument('--exclude-color-pages', '--skip-color-pages', dest='exclude_color_pages', default=None, type=str,
                        help='Pages to exclude from colorization (comma-separated page numbers/ranges e.g. 1,2,5-8, or filename patterns e.g. *cover*)')
    g_case = g_parser.add_mutually_exclusive_group()
    g_case.add_argument('--uppercase', action='store_true', help='Change translated text to uppercase (ALL CAPS)')
    g_case.add_argument('--lowercase', action='store_true', help='Change translated text to lowercase')
    g_parser.add_argument('--letter-case', '--text-case', dest='letter_case', default=None, choices=['upper', 'lower', 'none', 'original'],
                        help='Lettering case for translated text: upper, lower, or none/original')
...
parser_batch.add_argument('-i', '--input', required=True, type=str, nargs='+',
                          help='Image folder/file, ZIP/CBZ archive, or wildcard pattern')
...
parser_batch.add_argument('--colorize-only', action='store_true', help='Run only black and white colorization without translation, preserving speech bubbles.')
