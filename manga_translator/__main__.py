...
from manga_translator.args import expand_input_paths, parser, reparse
...
async def dispatch(args: Namespace):
    ...
    if args.mode == 'local':
        ...
        from manga_translator.mode.local import archive_output_path, is_archive_path
        translator = MangaTranslatorLocal(args_dict)
        input_paths = natural_sort(expand_input_paths(args.input))
        ...
        if len(input_paths) == 1 and os.path.isfile(input_paths[0]) and not is_archive_path(input_paths[0]):
            ...
            await translator.translate_path(input_paths[0], dest, args_dict)
            ...
            for path in input_paths:
                ...
                if is_archive_path(path):
                    archive_dest = archive_output_path(path, dest, len(input_paths) == 1)
                    await translator.translate_archive(path, archive_dest, args_dict)
                else:
