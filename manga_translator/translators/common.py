...
class OfflineTranslator(CommonTranslator, ModelWrapper):
    ...
    async def load(self, from_lang: str, to_lang: str, device: str):
        parsed_languages = self.parse_language_codes(from_lang, to_lang)
        await super().load(device, *parsed_languages)
        model = getattr(self, '_TRANSLATOR_MODEL', None) or getattr(self, '_CT2_MODEL_DIR', None)
        if model is None:
            model = getattr(self, '_CT2_MODEL_FOLDERS', {}).get('-'.join(parsed_languages), self.__class__.__name__)
        self.model_name = model
        self.logger.info(f'Using offline model: {model}')
