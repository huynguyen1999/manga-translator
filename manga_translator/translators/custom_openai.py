...
class CustomOpenAiTranslator(ConfigGPT, CommonTranslator):
    ...
    async def _translate(self, from_lang: str, to_lang: str, queries: List[str]) -> List[str]:
        ...
        self.logger.info(f"Calling Ollama API ({model or 'default'}) at {self.client.base_url} for {len(queries)} query/queries (target: {to_lang})")
        ...
        for prompt, query_size in self._assemble_prompts(from_lang, to_lang, queries):
            ...
            self.logger.info('-- Ollama Response (filtered) --\n' + response)
        ...
        self.logger.info(f'Ollama Translations: {translations}')
    ...
    async def _request_translation(self, to_lang: str, prompt: str) -> str:
        ...
        model = self.model or CUSTOM_OPENAI_MODEL
        self.logger.info(f"Sending request to Ollama API (model: {model or 'default'}) at {self.client.base_url}...")

        is_json_mode = getattr(self, "_professional_json_mode", False)
        kwargs = {
            "model": model,
            "messages": messages,
            "max_tokens": self._MAX_TOKENS if is_json_mode else self._MAX_TOKENS // 2,
