...
class GoogleTranslator(CommonTranslator):
    ...
    def _normalize_and_validate_lang(self, lang: str, is_dest: bool = False) -> str:
        lang = lang.lower().split('_', 1)[0]
        if not is_dest and lang == 'auto':
            return 'auto'
        ...
        label = 'destination' if is_dest else 'source'
        raise ValueError(f'invalid {label} language')

    def _build_translated_result(self, data, response, origin: str, src: str, dest: str) -> Translated:
    ...
    async def translate_legacy(self, text, dest='en', src='auto', **kwargs):
        ...
        """
        dest = self._normalize_and_validate_lang(dest, is_dest=True)
        src = self._normalize_and_validate_lang(src, is_dest=False)
        ...
        return self._build_translated_result(data, response, origin, src, dest)
