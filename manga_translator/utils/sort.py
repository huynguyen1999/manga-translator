...
def _assign_regions_to_panels(regions: List[TextBlock], panels: List[Tuple[int, int, int, int]]) -> None:
    for r in regions:
        cx, cy = r.center
        r.panel_index = -1
...
def _sort_regions_by_dispersion(
    regions: List[TextBlock],
    right_to_left: bool
) -> List[TextBlock]:
    if len(regions) <= 1:
    ...
    if x_std > y_std:
        return _sort_regions_horizontal(regions, right_to_left)
    return _sort_regions_vertical(regions, right_to_left)


def _sort_regions_horizontal(regions: List[TextBlock], right_to_left: bool) -> List[TextBlock]:
...
def _sort_regions_vertical(regions: List[TextBlock], right_to_left: bool) -> List[TextBlock]:
