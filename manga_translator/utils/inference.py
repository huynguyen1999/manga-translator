...
class ModelWrapper(ABC):
    ...
    def _get_loaded_torch_device(self):
    ...
    def _set_runtime(self, device=None, backend=None):
    ...
    async def reload(self, device: str, *args, **kwargs):
        await ModelWrapper.unload(self)
    ...
    async def load(self, device: str, *args, **kwargs):
        ...
        try:
            ...
            if not self.is_downloaded():
                await self.download()
            if self.is_loaded() and self._requested_device != requested_device:
                self.logger.info(
                    f'Reloading model for device change: model={self._key} '
                    f'requested_device={requested_device} previous_device={self._requested_device}'
                )
                await ModelWrapper.unload(self)
            if not self.is_loaded():
                ...
                self._loaded = True
                loaded_torch_device = self._get_loaded_torch_device()
                self._runtime_device = self._runtime_device or loaded_torch_device
                self._runtime_device = self._runtime_device or getattr(self, 'device', requested_device)
                self._runtime_backend = self._runtime_backend or (
                    'pytorch' if loaded_torch_device else 'unknown'
                )
                self.logger.info(
                    f'Model loaded: model={self._key} requested_device={requested_device} '
                    f'runtime_device={self._runtime_device} backend={self._runtime_backend}'
                )
        finally:
    ...
    async def infer(self, *args, **kwargs):
        ...
        self.logger.info(
            f'Model inference: model={self._key} requested_device={self._requested_device} '
            f'runtime_device={self._runtime_device} backend={self._runtime_backend}'
        )
