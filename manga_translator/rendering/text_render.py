...
def _evaluate_bubble_layout_candidates(
    ...
    max_font = max(int(font_size), min(int(round(font_size * 1.60)), int(height * 0.45), int(width * 0.50), 64))
...
def put_text_horizontal(font_size: int, text: str, width: int, height: int, alignment: str,
    ...
    if result is not None:
        ...
        line_text_list, line_width_list = calc_horizontal(
            font_size, text, width, height, lang, hyphenate, allow_width_expansion=False
        )
    ...
    line_box, bounding_mask = _render_horizontal_lines(
        ...
        reversed_direction, alignment, fg, bg
    )
