...
def _find_central_word_splits(words: List[str], word_lengths: List[int]):
    num_words = len(words)
    len_left, len_right = [], []
    ...
    if num_words > 1:
        ...
        central_index = 0
    return central_index, wlst_left, len_left, sum_left, wlst_right, len_right, sum_right


def _layout_central_line(
...
def _layout_bottom_half(
    ...
    while sum_right > 0:
        ...
        if new_x <= 0 or right_x >= bw:
            line_valid = False
        elif mask[pos_y:line_bottom, new_x].sum() > 0 or mask[pos_y:line_bottom, right_x].sum() > 0:
            line_valid = False
        else:
            line_valid = True
        if line_valid:
...
def layout_lines_aligncenter(
    ...
    (
        ...
        len_right,
        sum_right,
    ) = _find_central_word_splits(words, word_lengths)
    ...
    sum_left, sum_right = _layout_central_line(
        central_line, mask, centroid_x, pos_y, line_bottom, bw, delimiter_len, delimiter,
        max_central_width, wlst_left, len_left, sum_left, wlst_right, len_right, sum_right
    )

    lines = [central_line]

    _layout_bottom_half(
        lines, mask, centroid_x, centroid_y, line_height, bw, delimiter_len, delimiter,
        spacing, max_central_width, wlst_right, len_right, sum_right
    )

    _layout_top_half(
        lines, mask, centroid_x, centroid_y, line_height, bw, delimiter_len, delimiter,
        spacing, max_central_width, wlst_left, len_left, sum_left
    )

    return lines


def _adjust_region_enlarge_ratios(text_regions: List[TextBlock]):
    for region in text_regions:
        region.enlarge_ratio = 1
        region.enlarged_xyxy = region.xyxy.copy()

    def update_enlarged_xyxy(reg):
        reg.enlarged_xyxy = reg.xyxy.copy()
        w_diff, h_diff = ((reg.xywh[2:] * reg.enlarge_ratio) - reg.xywh[2:].astype(np.float64)) // 2
        reg.enlarged_xyxy[0] -= w_diff
        reg.enlarged_xyxy[2] += w_diff
        reg.enlarged_xyxy[1] -= h_diff
        reg.enlarged_xyxy[3] += h_diff
...
def _calculate_font_values(font_size: int, words: List[str], stroke_width: float, delimiter: str = ' '):
...
def _rotate_balloon_mask_if_needed(ballon_mask: np.ndarray, angle: float):
    if abs(angle) <= 3:
        return False, ballon_mask, 0, 0, 0.0, 0.0

    rotated = True
    region_angle_rad = np.deg2rad(angle)
    region_angle_sin = np.sin(region_angle_rad)
    region_angle_cos = np.cos(region_angle_rad)
    rotated_ballon_mask = np.array(Image.fromarray(ballon_mask).rotate(angle, expand=True))
    ...
    if 0 < norm_angle <= 90:
        ...
        ry = 0
    return rotated, rotated_ballon_mask, rx, ry, region_angle_sin, region_angle_cos


def _render_single_textblock_eng(
    img_pil: Image.Image,
    region: TextBlock,
    ...
    font_size, sw, line_height, delimiter_len, base_length, word_lengths = _calculate_font_values(
        region.font_size, words, stroke_width, delimiter
    )

    ballon_mask, xyxy = extract_ballon_region(original_img, region.xywh, enlarge_ratio=region.enlarge_ratio)
    if original_img is not None:
        region.layout_bounds = safe_ballon_bounds(original_img, region.xywh, enlarge_ratio=region.enlarge_ratio)

    rotated, ballon_mask, rx, ry, sin_a, cos_a = _rotate_balloon_mask_if_needed(ballon_mask, region.angle)
    ...
    if font_size_multiplier < 1:
        ...
        font_size, sw, line_height, delimiter_len, base_length, word_lengths = _calculate_font_values(
            font_size, words, stroke_width, delimiter
        )
    ...
    if rotated:
        rcx = rel_cx * cos_a - rel_cy * sin_a
        rcy = rel_cx * sin_a + rel_cy * cos_a
        rel_cx, rel_cy = rcx, rcy
...
def render_textblock_list_eng(
    ...
    _adjust_region_enlarge_ratios(text_regions)

    for region in text_regions:
        _render_single_textblock_eng(
            img_pil, region, original_img, delimiter, line_spacing,
            stroke_width, downscale_constraint
        )
